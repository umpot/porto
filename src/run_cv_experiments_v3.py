...
def Preprocessor(train, test):
    for col in hi_categoricals+comb_cols:
        print col
        train, test, tmp = hcc_encode(train, test, col, target, r_k=0)
...
grid_native = {
    ...
    'eval_metric': ['auc'],
    'max_depth':[3,4],
    'lambda':[0,15,10, 50, 100],
    'alpha':[0,1,5,10, 20, 50],
    'gamma':[0,1,5,10, 20],
    'min_child_weight':[0,1,2,5,10, 20, 50],
    'subsample':[0.8],
    'colsample_bytree':[0.8],
    'eta':[0.02],
    'scale_pos_weight':[1, 1.8]
}
name = 'grid_cv_res_v4_no_noise_grid_v2_seed_110'
