...
def add_interaction_col(df, features):
    col_name = 'interact_'+'_'.join(features)
    df[col_name] = ''
    for f in features:
        df[col_name]+="*"
...
def Preprocessor(train, test):
    ...
    for ff in combinations(to_comb, 2):
        col = add_interaction_col(train, ff)
        col = add_interaction_col(test, ff)
        train, test, tmp = hcc_encode(train, test, col, target)
...
def get_df_for_include_exclude_cols(df, target, include=None, exclude=None):
    if include is not None:
        cols = list(include) + [target]
...
def try_it(experiment_name, params_list):
    ...
    folds_list = create_folds_list(df, target, Preprocessor, 5, 1, seed)
...
params_list = [{'alpha': 10,
                ...
                'min_child_weight': 20,
                # 'num_boost_round': 327,
                'objective': u'binary:logistic',
                'scale_pos_weight': 1.5,
                'subsample': 0.8,
                'tree_method': u'hist'}]

name = 'trash_20_11_2017_x3'
print os.path.abspath(os.path.join('../out', '{}.hdf5'.format(name)))

try_it(name, params_list)
