...
def do_predict(out_fp, params_fp, seed):
    np.random.seed(seed)
...
def do_predict_for_different_seeds(out_fp, params_fp, seeds):
    with Hdf5Writer(out_fp, 'experiments', Experiment) as writer, open(params_fp) as f:
        params_list = json.load(f)
        pass


params_fp = '../params/submitting_params.json'
out_fp = 'x_seed666.hdf5'

seed =666
do_predict(out_fp, params_fp, seed)
